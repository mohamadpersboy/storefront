...
- پیاده‌سازی بصری با CSS Grid دو‌ستونه (`grid-cols-[1fr_2fr]` — عنوان
  ...
  خودِ ناحیه Grid یک بک‌گراند خاکستری روشن (`bg-gray-100`) دارد تا
  فاصله ۴px بین خانه‌های سفید واقعاً دیده شود (وگرنه روی بک‌گراند
  سفید هم‌رنگ کارت، آن فاصله‌ها قابل‌تشخیص نبودند).
- خانه‌ها: گوشه‌گرد + بک‌گراند سفید. ستون عنوان وسط‌چین
  (`text-center`)، ستون مقدار راست‌چین (`text-right` — همان جهت
  تحت‌اللفظی «راست‌چین» درخواست‌شده، نه `text-start` منطقی).
