...
کارفرما بعد از تأیید Bottom Bar): Mobile Top Bar.** بعد از دو دور
اصلاح طبق بازخورد دقیق کارفرما، نسخه نهایی:

- پس‌زمینه رنگی Indigo (`var(--color-primary)`) با ارتفاع کل حدود
  ۳۰٪ صفحه (ردیف آیکون + باکس جستجو + فضای خالی + شکم پایین).
- **لبه پایین به‌جای گوشه‌های گرد، یک «شکم» واقعی به سمت پایین دارد**
  ...
  منحنی به‌سمت پایین کشیده می‌شود. عمق رندرشده با `style={{height}}`
  کنترل می‌شود (نه با تغییر مسیر Bezier) — در دور دوم از ۴۴px به
  ۶۰px افزایش یافت (طبق درخواست «محسوس‌تر ولی نه خیلی زیاد»).
- آیکون سمت راست (start): **Wallet** (قابلیت واقعی موجود در Backend)
  → لینک به `/wallet` (صفحه هنوز ساخته نشده).
- آیکون سمت چپ (end): **Headphones** (پشتیبانی) → لینک به `/support`
  (صفحه هنوز ساخته نشده).
- باکس جستجو (`<form method="GET" action="/search">`) **مستقیماً
  زیر ردیف نام/آیکون‌ها** قرار دارد (نه پایین/Overlap روی شکم مثل
  تلاش اول) و کاملاً Pill-shaped است (`rounded-full`، طبق درخواست
  صریح «۵۰٪ گرد»). بدون هیچ Client JS (طبق اصل Performance: ترجیح
...
TS/ESLint/Vitest (۲۷۷)/Build همه سبز، هر سه بار (نسخه اول و دو دور
اصلاح).
