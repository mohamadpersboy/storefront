...
**اصلاح ظاهری Phase 1 (بعد از تأیید کارفرما با تصویر رفرنس):**
Restyle کامل `MobileBottomBar` طبق رفرنس دقیق کارفرما — Backdrop
شیشه‌ای/شفاف (`bg-white/75 backdrop-blur-xl`، بدون Border توپر)،
آیکون/متن غیرفعال خاکستری (`text-gray-400`) و **بدون Label**، Label
فقط زیر آیتم فعال نمایش داده می‌شود، رنگ آیتم فعال فعلاً **Indigo
همان Token دشبورد** (`var(--color-primary)`, #4f46e5) به‌جای پالت
`--sf-*` (کارفرما گفته «فعلاً» — یعنی موقتی است، بعداً ممکن است تغییر
کند)، ارتفاع بار از ۵۶px به ۶۸px افزایش یافت (`layout.tsx` هم با
`pb-[76px]` هماهنگ شد)، و روی آیکون فعال یک Glow محو با
...
کارفرما بعد از تأیید Bottom Bar): Mobile Top Bar.**

**تلاش اول رد شد کامل (Rejected → Revert):** نسخه پس‌زمینه رنگی
Indigo با ارتفاع ~۳۰٪ صفحه و «شکم» منحنی SVG در لبه پایین، بعد از دو
دور اصلاح جزئی، در نهایت توسط کارفرما کاملاً رد شد («نه اصلا راضی
نبودم، کلا پاکش کن»). طبق قانون پروژه (بخش ۱۰ Master Workflow:
Rejected → Revert → Redesign)، این تلاش کامل Revert شد
(commit `revert(storefront): remove rejected top bar design`) —
هیچ اثری از آن باقی نماند.

**نسخه فعلی (طراحی از صفر، بر اساس رفرنس جدید کارفرما):**
یک اپ مالی مینیمال با پس‌زمینه **سفید** (نه رنگی) و بدون هیچ منحنی
Hero-style:

- سمت راست (start در RTL): یک Badge برند دایره‌ای Indigo با متن
  «فس» (دو حرف اول «فرش سقطچی»، هم‌الگو با روش Initials موجود در
  `dashboard-shell.tsx`) — چون فایل Logo واقعی هنوز آپلود نشده؛
  جایگزین موقت Avatar پروفایل در رفرنس.
- سمت چپ (end در RTL): سه دکمه دایره‌ای خاکستری‌روشن (`bg-gray-100`)
  — Search، Bell (اعلان)، Headphones (پشتیبانی) — دقیقاً مطابق رفرنس.
- **مکانیزم RTL:** چون کل سایت از قبل `dir="rtl"` است، فقط با
  گذاشتن Logo اول و گروه آیکون‌ها دوم در DOM + `justify-between`،
  چیدمان به‌طور طبیعی MIRROR می‌شود (Logo راست، آیکون‌ها چپ) — بدون
  نیاز به `flex-row-reverse` یا کد اضافه.
- نقطه قرمز Badge اعلان روی آیکون Bell در رفرنس **عمداً حذف شد** —
  چون هیچ داده واقعی اعلان‌خوانده‌نشده در Backend وجود ندارد و
  نمایشش یک Badge ساختگی می‌بود (طبق همان اصل صفر واقعی Bottom Bar).
- بخش‌های دیگر رفرنس (Total Balance، دکمه‌های Top Up/Send/More،
  AI Insights، Transactions) **عمداً پیاده‌سازی نشدند** — کارفرما
  فقط «تاپ بار» (همان ردیف بالا) را خواسته بود، نه کل صفحه رفرنس.
...
`/search`، `/notifications`، `/support` هنوز صفحه ندارند —
Placeholder Navigation مثل الگوی Bottom Bar. تست‌ها:
TS/ESLint/Vitest (۲۷۷)/Build همه سبز.

**هماهنگ‌سازی Top Bar با Bottom Bar (بعد از دیدن نسخه Deploy‌شده در
موبایل واقعی، کارفرما گفت این دو با هم هماهنگ نیستند):**
- Backdrop هر دو یکسان شد: `bg-white/75 backdrop-blur-xl` +
  `border` محو (Top Bar قبلاً `bg-white` توپر بود).
- رنگ آیکون‌های Top Bar از `--sf-ink` تیره به `text-gray-400` تغییر
  کرد تا با رنگ حالت غیرفعال آیکون‌های Bottom Bar یکی باشد؛ ضخامت
  (`strokeWidth 1.75`) از قبل هم یکسان بود.
- نقطه Badge اعلان روی آیکون Bell اضافه شد (رنگ Indigo، هم‌رنگ حالت
  فعال Bottom Bar) — اما **نمایشش شرطی و پیش‌فرض خاموش است**
  (`hasUnreadNotifications = false`)، دقیقاً هم‌الگو با بج سبد خرید
  در Bottom Bar: رنگ مشخص شده طبق خواسته کارفرما، ولی تا API واقعی
  اعلان‌ها وصل نشود صفر/عدم‌نمایش واقعی است، نه ساختگی. برای فعال
  کردن، فقط این یک Boolean باید به داده واقعی وصل شود.

**نکته مهم غیرمرتبط با کد:** کارفرما یک اسکرین‌شات از موبایل واقعی
فرستاد که در آن به‌نظر می‌رسید Label «خانه» بالای صفحه هم افتاده.
این واقعاً باگ نبود — یک Artifact شناخته‌شده اسکرین‌شات
Full-Page کروم است که عناصر `position: fixed` (مثل Bottom Bar ما)
...
**پاپ‌آپ نوتیفیکیشن بازطراحی شد (رفرنس غنی‌تر کارفرما):** طبق
درخواست صریح («چون فعلا داده‌ای نداریم از Mock استفاده کن»)، آرایه
`MOCK_NOTIFICATIONS` در `notification-bell.tsx` اضافه شد — ۴ آیتم
Mock **مختص مشتری Storefront** (ارسال سفارش، کد تخفیف، تخفیف
محصول موردعلاقه، تأیید پرداخت) نه اعلان‌های ادمین‌محور رفرنس کارفرما
(که برای Dashboard مناسب‌تر بود). هر آیتم یک Badge آیکون رنگی
متفاوت دارد (آبی/بنفش/صورتی/سبز — طبق درخواست «رنگ‌های متنوع و
شاد»)، همراه نقطه Unread، Timestamp، و دکمه «علامت‌گذاری همه»
(Client-side، چون API واقعی نیست). موقعیت پاپ‌آپ از Absolute
نسبت‌به دکمه کوچک Bell به `fixed` با `left-4` (هم‌راستا با Padding
افقی صفحه) تغییر کرد تا واقعاً به حاشیه چپ بچسبد (طبق درخواست
صریح). یک فلش/مثلث کوچک CSS بالای پاپ‌آپ به دکمه Bell اشاره
می‌کند. فوتر «مشاهده همه اعلان‌ها» به `/notifications` (هنوز
نساخته) لینک می‌دهد. تست‌ها: TS/ESLint/Vitest (۲۷۷)/Build همه سبز.

**اصلاح جزئی موقعیت پاپ‌آپ (کارفرما محتوا را تأیید کرد، فقط فاصله):**
فاصله بالای پاپ‌آپ با Top Bar به صفر رساندیم (`top: safe-area +
68px`، دقیقاً ارتفاع خود Top Bar) تا مماس با لبه پایین آن باشد.
موقعیت فلش/مثلث از یک عدد تقریبی (`left-7`) به یک عدد محاسبه‌شده
(`left: 66px`) تغییر کرد تا دقیقاً زیر دکمه Bell بیفتد، نه جایی
وسط Search/Support — محاسبه بر اساس موقعیت واقعی گروه آیکون‌ها
(نگاه کنید کامنت بالای `notification-bell.tsx` برای فرمول کامل).

**پاپ‌آپ «پشتیبانی» جایگزین لینک ساده `/support` شد (درخواست صریح
کارفرما، بعد از اضافه‌شدن لینک‌های پیام‌رسان در «تنظیمات ← تماس با
ما»):** کامپوننت مستقل `src/components/storefront/support-popup.tsx`
(Client Component) — دقیقاً هم‌الگوی بصری/رفتاری `NotificationBell`
(همان شل Fixed/فلش/بستن با کلیک بیرون یا Escape)، اما به‌جای فهرست
اعلان، فهرست راه‌های تماس را نشان می‌دهد:

- داده از `ContactUs` (نه یک Fetch جدا در خود Component) در همان
  Server Component صفحه اصلی خوانده می‌شود (`getSupportContactSafe`
  در `page.tsx`، هم‌الگوی بقیهٔ Getterهای همان فایل) و به
  `MobileTopBar` → `SupportPopup` Prop داده می‌شود.
- هر ردیف فقط وقتی لینک/شماره مربوطه در Dashboard پر شده باشد
  نمایش داده می‌شود (`تماس تلفنی`، `گفتگو با پشتیبانی` =
  `supportAdminLink`، تلگرام، واتساپ، روبیکا، ایتا) — نه فقط یک
  آیکون تنها، بلکه آیکون رنگی + عنوان + توضیح، طبق درخواست صریح.
- آیکون‌ها با رنگ اختصاصی خودشان (نه یک‌رنگ یکسان): تلگرام/واتساپ
  رنگ رسمی برند (از همان `social-icons.tsx` موجود فوتر، برای
  جلوگیری از Duplicate SVG)؛ تماس تلفنی سبز (رنگ متعارف «تماس» در
  اپ‌های ایرانی، نه یک برند خاص)؛ گفتگو با پشتیبانی همان Indigo
  سازمانی پروژه (`--color-primary`)؛ روبیکا/ایتا چون رنگ رسمی
  تأییدشده‌ای در پروژه نداریم، تقریبی انتخاب شدند (کامنت بالای
  `buildItems` در خود فایل) — اگر بعداً رنگ رسمی مشخص شد، فقط همان
  دو مقدار باید عوض شوند.
- موقعیت فلش پاپ‌آپ: چون Support سومین/آخرین دکمه در گروه
  آیکون‌هاست (نه وسط مثل Bell)، `left: 14` محاسبه شد (یک عرض دکمه +
  Gap = ۵۲px کمتر از فلش Bell که `left: 66` است).

فایل‌ها: `support-popup.tsx` (جدید)، `mobile-top-bar.tsx` (Prop
`support` اضافه شد)، `(storefront)/page.tsx` (`getSupportContactSafe`
اضافه شد). تست‌ها: TS/ESLint/Vitest (۳۴۴)/Build همه سبز.
...
Category)/Build همه سبز. دو Commit مجزا Push شدند.

**رفع باگ واقعی مهم (چک‌باکس «نمایش در صفحه اصلی»):** کارفرما
گزارش داد با وجود تیک‌زدن چک‌باکس، دسته‌بندی در صفحه اصلی نمایش
داده نمی‌شود و در ویرایش دوباره، چک‌باکس همیشه خاموش دیده می‌شود.
**علت پیدا شد:** صفحه
`(dashboard)/dashboard/categories/[id]/edit/page.tsx` مقادیر
`imageUrl`/`imageBlurDataUrl`/`showOnHomepage`/`sortOrder` را اصلاً
به Prop اولیه (`initial`) کامپوننت `CategoryForm` پاس نمی‌داد — فقط
`name`/`slug`/`parentId`/`isActive`. یعنی هر بار صفحه ویرایش باز
می‌شد، چک‌باکس با `false` مقداردهی اولیه می‌شد (بی‌ربط به مقدار
واقعی در DB)، و اگر کاربر بدون توجه دوباره Submit می‌کرد، این مقدار
غلط `false` را روی DB می‌نوشت و مقدار درست قبلی را پاک می‌کرد.
رفع شد: همه این فیلدها حالا در `initial` پاس داده می‌شوند.
همچنین محتمل بود بخشی از سردرگمی کارفرما به‌خاطر `revalidate = 60`
(ISR) صفحه اصلی هم بوده — تغییرات تا ۶۰ ثانیه دیرتر دیده می‌شوند.

**فیچر جدید: اولویت نمایش در صفحه اصلی.** تا الان هیچ UI ای برای
تنظیم `sortOrder` (که از قبل روی مدل بود و در Query صفحه اصلی هم
استفاده می‌شد) وجود نداشت. یک ورودی عددی «اولویت نمایش در صفحه
اصلی» به `category-form.tsx` اضافه شد — فقط وقتی چک‌باکس «نمایش در
صفحه اصلی» فعال باشد نمایش داده می‌شود (چون فقط در آن حالت معنا
دارد). عدد کوچک‌تر = نمایش زودتر (راست‌تر، طبق ترتیب RTL).
...
**رفع باگ ریشه‌ای CategoryShortcuts (تلاش قبلی «اصلاح» Auto-Scroll
خودش اشتباه بود):** کارفرما گزارش داد تصاویر هنوز حرکت نمی‌کنند و
اولین دسته‌بندی‌ها به‌جای راست، انتهای Scroll هستند (باید به چپ
اسکرول می‌کرد تا آن‌ها را ببیند). علت: تلاش قبلی برای «حل ناهماهنگی
scrollLeft در RTL» با اجبار `dir="ltr"` روی ظرف Scroll + Reverse
کردن آرایه دسته‌ها اشتباه بود. طبق مشخصات استاندارد CSSOM View،
`dir="rtl"` طبیعی (بدون هیچ Override یا Reverse ای) خودش دقیقاً
رفتار درست را می‌دهد: `scrollLeft` از ۰ (لبه راست/شروع محتوا) تا
`-(scrollWidth - clientWidth)` (لبه چپ/پایان) منفی می‌شود — یعنی
با `scrollLeft` پیش‌فرض ۰، دسته اول از ابتدا در راست‌ترین جای دیده
می‌شود، بدون نیاز به هیچ Reverse. Auto-Scroll هم برای حرکت درست
باید به سمت منفی برود (`directionRef` اولیه از `1` به `-1` تغییر
کرد و مرزهای برخورد هم متناظر آن). این هم Reverse حذف شد هم منطق
جهت اصلاح شد — نه فقط یک Patch روی نشانه‌های ظاهری.

**اصلاح بنر ارسال رایگان:** آیکون از سمت چپ به راست منتقل شد (در
DOM اول آمد؛ چون صفحه RTL است، اولین فرزند Flex در راست می‌نشیند).
فونت‌ها کوچک‌تر شدند (`text-sm`→`text-xs`, `text-xs`→`text-[11px]`)
و Padding از `p-4` به `p-3.5` کم شد تا ارتفاع بنر بی‌جهت زیاد نشود
(طبق بازخورد صریح کارفرما).

تست‌ها: TS/ESLint/Vitest (۲۹۶)/Build همه سبز.

**رفع نهایی Auto-Scroll (سومین و آخرین تلاش — با تشخیص Runtime،
نه حدس):** کارفرما بعد از تلاش دوم گزارش داد باز هم کاملاً متوقف
است («قبلاً حرکت می‌کرد و سریع بود؛ حالا که خواستی آرومش کنی کلاً
متوقفش کردی»). ریشه هر دو شکست قبلی یکی بود: **فرض غلط درباره
Convention عددی `scrollLeft` در RTL** — تلاش اول فرض کرد باید با
`dir="ltr"` دور زد (اشتباه، باعث نمایش دسته اشتباه در ابتدا شد)،
تلاش دوم فرض کرد مشخصات استاندارد CSSOM View (`scrollLeft` منفی)
در عمل هم صادق است (اشتباه بود — این مرورگر خاص مقدار منفی را
Clamp به صفر می‌کرد، یعنی هیچ حرکتی هرگز ثبت نمی‌شد). **راه‌حل
نهایی: دیگر حدس نمی‌زنیم.** یک تابع `detectRangeSign` هنگام Mount
یک‌بار امتحان می‌کند `scrollLeft` را با ۲۰+ افزایش دهد؛ اگر واقعاً
تغییر کرد (Clamp نشد)، بازه معتبر `[0,+max]` است؛ وگرنه `[-max,0]`
امتحان می‌شود. این دقیقاً همان تکنیک صنعتی استاندارد
(Bootstrap RTL / jQuery scrollLeft fix) برای همین مشکل مستند است.
رفت‌وبرگشت Auto-Scroll هم با همین بازه تشخیص‌داده‌شده محاسبه
می‌شود، نه یک فرض ثابت. سرعت (`AUTO_SCROLL_PX_PER_FRAME = 0.15`)
دست‌نخورده ماند — طبق تأکید صریح کارفرما («فقط سرعتش رو کم کن»)،
هیچ تغییر دیگری در رفتار/فاصله اعمال نشد.

**اصلاح آیکون بنر ارسال رایگان:** آیکون Truck پیش‌فرض Lucide رو به
چپ طراحی شده؛ با `-scale-x-100` افقی Mirror شد تا رو به راست باشد.

تست‌ها: TS/ESLint/Vitest (۲۹۶)/Build همه سبز.

**Phase 4: Special Offers Carousel («شگفت‌انگیزها») — دور دوم بازخورد
دیداری اعمال شد (هنوز Mock).**

- **رنگ برند بخش:** `--sf-cherry` از قرمز آلبالویی سفارشی به دقیقاً
  Tailwind `rose-600`/`rose-50` تغییر کرد (`#e11d48`/`#fff1f2`).
  `SectionHeader` حالا `seeAllClassName` هم دارد (رنگ+سایز فونت لینک
  «مشاهده بیشتر» به‌طور کامل قابل بازنویسی — این بخش را کوچک‌تر و
  هم‌رنگ Cherry کرد).
- **باگ واقعی اندازهٔ متفاوت تصاویر (پیدا و رفع شد):** توضیح غلط
  دور قبل («مشکل از تصویر Mock بود») اشتباه بود. علت واقعی:
  `first:ps-0` روی اولین کارت فقط یک‌طرف Padding را حذف می‌کرد، پس
  عرض محتوای همان یک کارت بیشتر از بقیه می‌شد و چون تصویر با
  `aspect-[3/4]` وابسته به عرض بود، فقط همان کارت بلندتر دیده
  می‌شد. رفع شد با: (۱) حذف `first:ps-0` (همهٔ کارت‌ها Padding
  یکسان دارند)، (۲) اندازهٔ تصویر دیگر با `aspect-ratio` نسبی نیست،
  با پیکسل قطعی (`w-[128px] h-[171px] sm:w-[144px] sm:h-[192px]`)
  تعیین شده — دیگر به هیچ Padding/عرض اطراف وابسته نیست.
- **فلاش نوار سفید پایین صفحه هنگام Scroll (پیدا و رفع شد):** علت،
  ناهماهنگی پس‌زمینهٔ واقعی `<body>` (سفید، سراسری برای کل پروژه)
  با پس‌زمینهٔ خاکستری Storefront بود؛ وقتی نوار آدرس مرورگر موبایل
  هنگام Scroll جمع/باز می‌شود، ارتفاع Viewport لحظه‌ای تغییر می‌کند
  و این ناهماهنگی رنگ برای یک لحظه دیده می‌شد. رفع شد با: (۱)
  Selector جدید `body:has(> .storefront)` در `globals.css` که فقط
  برای صفحات Storefront پس‌زمینهٔ `<body>` را هم خاکستری می‌کند
  (Dashboard دست‌نخورده)، (۲) `min-h-screen` → `min-h-dvh` در
  Layout ریشهٔ Storefront (`(storefront)/layout.tsx`).
- **دور سوم بازخورد (تایمر/Progress Bar):** شمارش معکوس حالا
  `dir="ltr"` صریح دارد (بدون این، داخل صفحهٔ RTL ترتیب
  ساعت/دقیقه/ثانیه از راست‌به‌چپ می‌خواند و برعکس دیده می‌شود)، رنگش
  Cherry شد و چپ‌چین (`text-left`) قرار گرفت. Progress Bar: منطق
  برعکس شد — به‌جای «زمان باقی‌مانده» (که خالی می‌شد) حالا «زمان
  سپری‌شده» را نشان می‌دهد و به‌مرور پر می‌شود؛ پس‌زمینهٔ مسیر از
  `bg-gray-100` (که چون دقیقاً هم‌رنگ بک‌گراند صفحه بود عملاً نامرئی
  می‌شد و طول واقعی نوار مشخص نبود) به `--sf-cherry-soft` (صورتی
  کم‌رنگ، همیشه قابل‌دیدن) تغییر کرد.
...
**Phase 9: Footer — انجام شد (زودتر از ترتیب اصلی، طبق دستور صریح
کارفرما؛ Phase 6/2 هنوز کار نشده‌اند).**

- `Footer` (`footer.tsx`): نام فروشگاه + تگ‌لاین، ردیف آیکون
  شبکه‌های اجتماعی، لینک‌های سریع (درباره ما/تماس با ما/سوالات
  متداول/قوانین و مقررات — موقتاً به مسیرهای هنوز طراحی‌نشده)،
  جای‌نگه‌دارِ صادقانهٔ نماد اعتماد الکترونیکی (نه نشان جعلی — تا
  دریافت مجوز رسمی از enamad.ir)، خط کپی‌رایت.
- **آیکون‌های شبکه‌های اجتماعی داده‌محورند، نه Mock:** چون قابلیت
  Social Links در Backend/Dashboard از قبل کامل و آماده است
  (`SocialLinks` Model + `getSocialLinks()`)، طبق بند ۱۳ Master
  Workflow («فقط جایی Mock بزن که API واقعی آماده نیست») مستقیم به
  DB وصل شد — نه Mock. `page.tsx` مثل بنرها/دسته‌بندی‌ها مستقیم
  `getSocialLinks()` را می‌خواند، فقط پلتفرم‌هایی که هم `isActive`
  و هم `url` غیرخالی دارند فیلتر و به `Footer` پاس داده می‌شوند.
  **نکته مهم برای بررسی:** اگر فعلاً در Dashboard → تنظیمات →
  شبکه‌های اجتماعی هیچ پلتفرمی فعال/دارای URL نباشد، ردیف آیکون‌ها
  اصلاً نمایش داده نمی‌شود (رفتار عمدی، نه باگ) — برای دیدن
  آیکون‌ها باید حداقل یکی را از همان‌جا فعال کنید.
- `social-icons.tsx`: چون Lucide اصلاً برای Rubika/Eitaa (و اکثر
  شبکه‌های اجتماعی دیگر) آیکون ندارد، ۵ SVG ساده و مستقل نوشته شد
  (Instagram, Telegram, WhatsApp, Rubika, Eitaa) — بازسازی
  ساده‌شدهٔ شکل کلی هر اپ، نه کپی دقیق لوگوی رسمی؛ اگر بعداً فایل
  SVG رسمی برند در اختیار باشد، به‌راحتی جایگزین می‌شود.
...
**Phase 8: ردیف ویژگی‌ها + کارت «درباره ما» — انجام شد.**

- `FeaturesRow` (`features-row.tsx`): ۴ ویژگی Static (پشتیبانی،
  پرداخت در محل، ضمانت اصالت، ارسال سراسری) — محتوای گرید مثل
  رفرنس بصری کارفرما، اما رنگ آیکون‌ها آبی برند خودمان
  (`--sf-accent`) است، نه سبز رفرنس (آن فقط الگوی چیدمان بود).
  «تازگی تضمینی»/«همان‌روز» رفرنس (مخصوص میوه/سبزی) با «ضمانت
  اصالت»/«ارسال سراسری» مناسب فرش جایگزین شد تا با
  `FreeShippingBanner` هم تکراری نباشد.
- `AboutUsCard` (`about-us-card.tsx`): **داده واقعی**، نه Mock —
  چون Dashboard → تنظیمات → درباره ما از قبل کامل بود
  (`getAboutUs()`)، `page.tsx` مثل Footer/بنرها مستقیم از DB
  می‌خواند. متن با `line-clamp-3` خلاصه می‌شود؛ «بیشتر بدانید» به
  همان `/about` که لینک «درباره ما» در Footer هم اشاره می‌کند.
  پس‌زمینهٔ تیره از `--sf-ink` (نه رنگ جدید اختیاری).
- طبق دستور صریح کارفرما، سایز فونت هیچ‌جای این دو کامپوننت از
  بقیهٔ صفحه بزرگ‌تر نیست (برخلاف رفرنس بصری که تیترهای درشت‌تری
  داشت).
- مارجین بالای Footer دوباره بازبینی شد: `mt-3` (۱۲px) → `mt-6`
  (۲۴px).
...
- ✅ **بازطراحی «آدرس‌های من» و «اطلاعات بانکی» طبق بازخورد بصری
  ...
  **کپی دقیق آن رفرنس‌ها نباشد**، بلکه ایده‌ها گرفته شود و طراحی
  «بهتر و جالب‌تر» و فرم‌ها «فانتزی‌تر (بدون زیاده‌روی)» شوند.

  **تغییر مدل آدرس (Breaking، عمدی):** `Address.title` (Input آزاد)
  حذف و با `addressType: "home" | "work" | "other"` +
  `customTitle` (فقط برای `"other"`) جایگزین شد. چون این فیلد همین
  امروز در همین Session اضافه شده بود و هنوز هیچ کاربر واقعی از آن
  استفاده نکرده، این یک Refactor زودهنگام است، نه شکستن داده
  Production — طبق قاعده «تغییر افزایشی در مدل‌های Production»، اگر
  این فیلد قبلاً واقعاً در تولید استفاده می‌شد، به‌جایش یک فیلد
  جدید کنار `title` اضافه می‌شد.
  - `lib/utils/address.ts` → `getAddressDisplayTitle()`: برچسب
    «خانه»/«محل کار» را از کد می‌سازد (نه از Database)، برای
    `"other"` همان `customTitle` را برمی‌گرداند.
  - `AddressForm`: بجای Input عنوان، سه دکمه انتخاب نوع (آیکون
    Home/Briefcase/MapPinned + رنگ‌بندی سبز/آبی/بنفش) — فقط برای
    «سایر» Input عنوان دلخواه باز می‌شود.
  - `AddressListItem`: بازطراحی کامل به‌شکل کارت با آیکون رنگی نوع
    آدرس، Badge «پیش‌فرض»، و یک ردیف اقدام مستقیم روی خود کارت
    (حذف/ویرایش/تعیین پیش‌فرض) — دیگر لازم نیست کاربر برای حذف یا
    پیش‌فرض‌کردن، وارد فرم ویرایش شود.

  **کارت اطلاعات بانکی:** `CustomerBankAccount` یک فیلد اختیاری
  `bankName` (رشته آزاد ساده) گرفت — عمداً ارجاع به مدل `Bank`ِ
  داشبورد نشد چون آن Endpoint (`GET /api/v1/banks`) پشت Permission
  ادمین (`BANKS_READ`) است و برای مشتری Storefront در دسترس نیست؛
  ساختن نسخه عمومی از آن فقط برای نمایش نام بانک روی یک کارت،
  بزرگ‌تر از حد این قابلیت بود.
  - `DELETE /api/v1/account/bank-info` اضافه شد (قبلاً فقط
    GET/PUT بود).
  - کامپوننت‌های جدید: `BankInfoCard` (نمایش گرادیانی با دکمه
    چشم برای نمایش/مخفی‌کردن شماره کامل + دکمه‌های کپی جدا برای
    کارت/شبا) و `BankInfoSection` (سوییچ بین نمای کارت و فرم —
    وقتی رکورد ذخیره‌شده وجود دارد نمای پیش‌فرض «کارت» است، فرم
    فقط با زدن «ویرایش» یا برای اولین‌بار باز می‌شود).

  تست‌ها به‌روزرسانی شدند: `addresses.test.ts` (۱۱ تست، شامل
  الزامی‌بودن `customTitle` برای `"other"`)، `bank-info.test.ts`
...
- [x] Phase 8: سایر بخش‌های موردنیاز Homepage — ردیف ویژگی‌ها +
  کارت «درباره ما» (داده واقعی از `getAboutUs()`) انجام شد — نگاه
  کنید بخش ۲
- [x] Phase 9: Footer + شبکه‌های اجتماعی — انجام شد، زودتر از Phase
  2/6/8 (طبق دستور صریح کارفرما؛ نگاه کنید بخش ۲). یادداشت قدیمی
  این خط («Settings جدید باید ساخته شود») دیگر درست نیست — بخش
  Settings → شبکه‌های اجتماعی از قبل در Dashboard کامل بود؛ Footer
  مستقیم همان را می‌خواند.
...
- **رفع شد — پاپ‌آپ «ثبت دریافت وجه» خودش اسکرول نمی‌شد (پنجرهٔ اصلی
  پشت آن اسکرول می‌شد و بخشی از بالای فرم دیده نمی‌شد):** الگوی این
  Modal با بقیهٔ Modalهای طولانی پروژه (مثلاً `banner-form-modal.tsx`،
  `brand-form-modal.tsx`) فرق داشت — `overflow-y-auto` روی خودِ Overlay
  بیرونی (`fixed inset-0`) گذاشته بود و پنل داخلی سقف ارتفاع نداشت.
  **الگوی درست و همیشگی پروژه:** Overlay بیرونی فقط `p-4` (بدون
...
- **رفع شد — نام برند در لیست موبایل صفحهٔ Brands اصلاً دیده نمی‌شد
  ...
  Flex Row تک‌خطی بود با تعداد زیادی کنترل ثابت‌عرض (بج‌ها، دکمه‌های
  بالا/پایین/فعال‌سازی/ویرایش)؛ چون فقط بلوک نام/اسلاگ `min-w-0` داشت،
  در رقابت فضا روی موبایل به عرض صفر فشرده می‌شد. راه‌حل: ردیف به
  `flex-wrap` تغییر کرد و بلوک «تصویر + نام/اسلاگ» با `basis-full` روی
  موبایل خط مستقل خودش را می‌گیرد (همیشه کامل دیده می‌شود)، و بج‌ها/
  اکشن‌ها روی خط بعدی می‌شکنند؛ از `sm` به بالا دوباره یک‌خطی می‌شوند.

- **رفع شد — چهار باگ UI صفحه جزئیات سفارش/ثبت پرداخت (چک، Date
  Picker، دکمه، اقلام سفارش):**
  - **حاشیه دوتایی/گوشه‌گرد روی اقلام سفارش (موبایل):** چون Table
    مستقیماً داخل یک `Card` با حاشیه خودش قرار داشت (نه `TableCard`)،
    استایل موبایلِ هر `<tr>` (`.responsive-table tr` — حاشیه + گوشه‌گرد
    مستقل) روی حاشیهٔ Card سوار می‌شد. راه‌حل: پراپ `flat` روی
    `Table` (کلاس `.responsive-table--flat` در `globals.css`) که
    حاشیه/گوشه‌گرد/پس‌زمینه هر ردیف را حذف می‌کند و فقط یک خط جداکننده
    پایین می‌گذارد — ردیف‌ها به‌جای کارت جدا، مثل یک لیست به‌هم‌پیوسته
    دیده می‌شوند. **قاعده کلی:** هر `Table`ی که از قبل داخل یک `Card`
    معمولی (نه `TableCard`) قرار می‌گیرد باید `flat` بگیرد.
  - **دکمهٔ «ثبت دریافت وجه» روی موبایل بریده می‌شد:** روی موبایل
    فقط «دریافت» نمایش داده می‌شود (`hidden sm:inline` / `sm:hidden`).
    به‌طور کلی‌تر، `CardHeader` هم اصلاح شد (`min-w-0 flex-1` روی
    بخش عنوان/توضیح، `shrink-0` روی `action`) تا توضیح طولانی، دکمهٔ
    کنار خودش را از صفحه بیرون نراند.
  - **انتخاب چک از بین همهٔ چک‌ها (Combobox با فقط ۵۰ چک اول، فیلتر
    سمت کلاینت) با رشد تعداد چک‌ها غیرعملی است:** کامپوننت جدید
    `src/components/checks/check-picker.tsx` (`CheckPicker`) ساخته
    شد — دقیقاً هم‌الگو با `AdminPicker` — که با Debounce به
    `/api/v1/checks?search=...` (پارامتر `search` از قبل در API
    پشتیبانی می‌شد) وصل می‌شود و جستجوی واقعی سمت سرور انجام می‌دهد؛
    جایگزین Combobox قدیمی در فرم «ثبت دریافت وجه» (`checkMode ===
    "existing"`) شد. با باز شدن فیلد (قبل از تایپ چیزی)، ۵ چک آخرِ
    ثبت‌شده هم به‌طور پیش‌فرض نمایش داده می‌شوند تا انتخاب سریع بدون
    جستجو هم ممکن باشد.
  - **هم‌افتادگی «تاریخ دریافت» و «تاریخ سررسید» روی موبایل:** گرید
    دو ستونهٔ این دو `JalaliDatePicker` در `manual-payments-panel.tsx`
    به `grid-cols-1 sm:grid-cols-2` تغییر کرد — همان الگویی که در
    `check-form.tsx` از قبل درست بود؛ این یکی جا افتاده بود.
...
**ردیف‌های محصول اختصاصی برای دسته‌بندی‌های اولویت صفر اضافه شد.**
طبق درخواست صریح کارفرما («هر دسته‌بندی‌ای که اولویت صفر بهش داده
شده باشه رو ... مثل بخش پرفروش‌ها و جدیدترین‌ها ... کروسل داشته
باشه»):

- تابع جدید `getPriorityCategorySections` در
  `src/lib/storefront/homepage-products.ts` — دسته‌بندی‌های سطح
  اول را می‌گیرد که هم `showOnHomepage` دارند و هم «اولویت نمایش»
  (فیلد `sortOrder`، همان که در فرم Dashboard برچسب «اولویت نمایش
  در صفحه اصلی» دارد) دقیقاً `0` است.
- **فرض صریح (مستندشده اینجا چون در درخواست دقیق مشخص نشده بود):**
  «اولویت صفر» یعنی همان `sortOrder === 0` — همان فیلدی که برای
  ترتیب ردیف دسته‌بندی‌های صفحه اصلی (`CategoryShortcuts`) هم
  استفاده می‌شود؛ مقدار پیش‌فرض این فیلد هم `0` است، پس این ویژگی
  فقط برای دسته‌بندی‌هایی فعال می‌شود که علاوه‌بر `showOnHomepage`،
  این فیلد را دست‌نخورده/صفر رها کرده باشند یا عمداً `0` گذاشته
  باشند.
- محصولات هر دسته = محصولات متصل به خودِ آن دستهٔ سطح اول **یا** به
  یکی از زیردسته‌های مستقیمش (`buildCategoryIdGroups`، تابع خالص با
  تست Unit مستقل) — چون در فرم محصول، دسته‌بندی می‌تواند یک زیردسته
  هم باشد (گزینه‌های با پیشوند «⤷»).
- هر دسته، جدیدترین محصولات منتشرشدهٔ خودش را نشان می‌دهد (همان
  منطق مرتب‌سازی «جدیدترین‌ها»)؛ دسته‌ای که فعلاً هیچ محصول
  قابل‌نمایشی ندارد از خروجی حذف می‌شود (هیچ ردیف خالی رندر
  نمی‌شود).
- Component جدید `CategoryProductsSection` — دقیقاً هم‌شکل
  `BestSellersSection`/`LatestProductsSection` (همان `ProductCard`،
  هدر مشکی بدون رنگ برند اختصاصی، آیکون عمومی `LayoutGrid`).
  `page.tsx` تعداد نامشخصی از این Component را با `.map` روی خروجی
  `getPriorityCategorySectionsSafe` (Wrapper با `try/catch`، هم‌الگو
  با بقیهٔ ردیف‌ها) رندر می‌کند، بعد از «جدیدترین‌ها» و پیش از
  `FeaturesRow`.
- `seeAllHref` هر بخش موقتاً به `/products/category/[slug]` اشاره
  می‌کند؛ آن صفحه هنوز طراحی نشده است.

تست‌ها: TS/ESLint/Vitest (۳۳۸ تست، ۳ تست جدید
`buildCategoryIdGroups`)/Build همه سبز.

**فیلد «اولویت نمایش» به محصول اضافه شد و در تمام ردیف‌های
Storefront اعمال می‌شود.** طبق درخواست صریح کارفرما («مثل همین
اولویت دسته‌بندی‌ها ... عدد کمتر یعنی اولویت بیشتر»):

- `Product.sortOrder` (عدد، پیش‌فرض `0`) — دقیقاً هم‌نام و هم‌معنا
  با `sortOrder` در Category/Banner/Brand؛ در فرم محصول (بخش
  «افزودن/ویرایش محصول») به‌عنوان «اولویت نمایش در فروشگاه» قابل
  تنظیم است، با همان متن راهنمای Category («عدد کوچک‌تر زودتر
  نمایش داده می‌شود»).
- در `src/lib/storefront/homepage-products.ts`، تابع خالص جدید
  `sortByPriority` (مرتب‌سازی *پایدار* صعودی، `?? 0` برای محصولات
  بدون این فیلد؛ ۴ تست Unit) به‌عنوان یک لایهٔ مرتب‌سازی *اول* روی
  معیار طبیعی هر ردیف اعمال می‌شود — یعنی اولویت جایگزین معیار
  قبلی نمی‌شود، فقط جلوتر می‌ایستد؛ در تساوی اولویت، همان ترتیب
  قبلی (جدیدترین/پرفروش‌ترین/نزدیک‌ترین به پایان Offer) حفظ
  می‌شود:
  - جدیدترین‌ها → اولویت، سپس تاریخ ایجاد
  - پرفروش‌ترین‌ها → اولویت، سپس مجموع فروش واقعی
  - شگفت‌انگیزها → اولویت، سپس نزدیک‌ترین به پایان Offer
  - ردیف‌های دسته‌بندی اولویت-صفر → اولویت، سپس تاریخ ایجاد
- **نکتهٔ فنی مهم (چرا مستقیم در Query خودِ Mongo مرتب‌سازی
  نشد):** Mongo مقدار *نبودِ* یک فیلد در Sort را «کوچک‌تر از هر
  عدد» حساب می‌کند، نه معادل `0` — یعنی اگر مستقیم
  `.sort({sortOrder:1,...})` می‌زدیم، محصولات قدیمی‌ای که این
  فیلد را اصلاً ندارند (قبل از این تغییر ساخته شده‌اند) همیشه
  جلوتر از محصولاتی می‌افتادند که عمداً `sortOrder: 0` گرفته‌اند —
  دقیقاً همان دستهٔ باگ «Mongoose schema defaults don't backfill»ی
  که قبلاً چند بار در این پروژه رفع شده. راه‌حل: هر تابع یک
  Candidate Pool بزرگ‌تر (۴ برابر، `PRIORITY_OVER_FETCH_MULTIPLIER`)
  با همان معیار طبیعی قبلی می‌گیرد، بعد در جاوااسکریپت با `?? 0`
  مرتب می‌شود، سپس به تعداد نهایی برش می‌خورد.
- مدل/Validation/فرم/صفحهٔ ویرایش همه به‌روزرسانی شدند
  (`Product.sortOrder`، `createProductSchema`، `ProductForm`،
  `EditProductPage` با `product.sortOrder ?? 0`).

تست‌ها: TS/ESLint/Vitest (۳۴۲ تست، ۴ تست جدید
`sortByPriority`)/Build همه سبز.

**فیلد «اولویت» به هر Variant محصول اضافه شد — قیمت کارت/صفحه محصول
اکنون بر اساس این اولویت انتخاب می‌شود.** طبق درخواست صریح کارفرما
(«اون ورینتی که عدد کمتری داره اولویت بیشتری داشته باشه»، هم‌معنا با
اولویت Product/Category موجود:

- `IProductVariant.sortOrder` (عدد، پیش‌فرض `0`) — دقیقاً هم‌الگو با
  `Product.sortOrder`؛ Variantهای قدیمی‌تر بدون این فیلد باید همیشه
  با `?? 0` خوانده شوند (همان قاعدهٔ همیشگی «Mongoose defaults don't
  backfill»).
- `pickRepresentativeVariant` (منبع واحد انتخاب Variant نماینده —
  هم روی کارت‌های صفحه اصلی، هم روی قیمت صفحه جزئیات محصول استفاده
  می‌شود) حالا **اول** بر اساس این اولویت مقایسه می‌کند (عدد
  کوچک‌تر برنده است)؛ فقط در تساوی اولویت به معیار قبلی (ارزان‌ترین
  Variant بعد از تخفیف) برمی‌گردد — یعنی رفتار قبلی برای محصولاتی
  که همهٔ Variantهایشان اولویت پیش‌فرض `0` دارند (تقریباً همهٔ
  محصولات فعلی) بدون تغییر می‌ماند.
- UI: فیلد «اولویت Variant» به `VariantEditor` اضافه شد (همان متن
  راهنمای اولویت دسته‌بندی: «عدد کوچک‌تر یعنی اولویت بیشتر»).
- **محدودیت فعلی مستندشده:** صفحهٔ جزئیات محصول هنوز هیچ سوییچر
  دیداری بین Variantها ندارد (طبق Scope فعلی Phase — فقط
  عنوان/قیمت). این تغییر تعیین می‌کند «کدام Variant پیش‌فرض انتخاب و
  قیمتش نشان داده شود»، اما نمایش/انتخاب دستی سایر Variantها توسط
  خود کاربر همچنان یک ماژول جداگانه و آینده است.
...
**بنر «دعوت دوستان» در `/account` بازطراحی شد** — طبق اسکرین‌شات
رفرنس دقیق دومِ کارفرما (صفحه «حساب من» یک اپ دیگر): به‌جای یک
ردیف ساده داخل لیست «خرید و سفارش‌ها»، حالا یک بنر گرادیانی
تمام‌عرض درست زیر کارت پروفایل است (`ReferralAccountBanner`) —
چیدمان دقیقاً مثل رفرنس (آیکون هدیه سمت راست در جعبه نیمه‌شفاف،
عنوان/توضیح وسط، فلش سمت چپ)، فقط با گرادیان `--sf-accent` →
`--sf-ink-soft` خودمان به‌جای بنفش رفرنس. هنوز فقط وقتی
`ReferralSettings.enabled` باشد نمایش داده می‌شود.

**بررسی گزارش خطای ۴۰۴ روی لینک `/account/referral`:** مسیر در
Build واقعاً ساخته می‌شود (`ƒ /account/referral` در خروجی
`next build`) و `src/proxy.ts` (تنها Middleware پروژه) فقط روی
`/dashboard/:path*` اثر دارد — پس هیچ Route Guard ای این مسیر را
مسدود نمی‌کند. علت محتمل: تأخیر/شکست Deploy خودکار Vercel برای
Commitِ قبلی، یا تست قبل از فعال‌کردن `enabled` در تنظیمات (که
باعث پیام «در دسترس نیست» می‌شود، نه ۴۰۴ واقعی). نیاز به تأیید
