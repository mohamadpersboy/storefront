...
- **عمداً اضافه نشد (طبق دستور صریح کارفرما):**
  - `technicalDescription` (متن آزاد فنی) — کارفرما در فاز بعدی
    فقط دستور جدول `technicalSpecifications` را داد (نگاه کنید
    `ProductTechnicalSpecsCard` پایین‌تر)؛ این یکی هنوز منتظر دستور.
