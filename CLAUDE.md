...
**اصلاح ظرافت رفلکس (دور دوم بازخورد، بعد از دیدن رفلکس واقعی روی
موبایل):** ارتفاع از ۴۰-۵۶px به ~۱۰px (`h-2.5`/`h-3`) کم شد، Blur
از `blur-xl` به `blur-2xl` افزایش یافت، Opacity از `0.12` به `0.20`
کمی بالا رفت، و Mask از یک Gradient خطی (فقط عمودی، لبه‌های چپ/راست
تیز) به یک `radial-gradient` بیضی‌شکل تغییر کرد تا لبه‌ها گرد و
