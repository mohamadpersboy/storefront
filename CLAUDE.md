...
- **Focus مشترک بین همه تصاویر (نه فقط تصویر Tap‌شده — به درخواست
  صریح کارفرما، جایگزین طراحی قبلی که Focus فقط برای همان یک اسلاید
  بود):** ضربه‌زدن (Tap با آستانه جابه‌جایی ۶px تا بعد از
  اسکرول/Pinch/Pan با انگشت به‌اشتباه Tap تشخیص داده نشود) روی هر
  تصویری، همه تصاویر گالری را با هم «Focus» می‌کند (State مشترک
  `isFocused: boolean`، نه `focusedIndex` جداگانه): عرض همه اسلایدها
...
- **Tap دوباره روی هر تصویری، وقتی همه Focus‌اند، همه را با هم
  ...
  می‌کند (`setZoom(null)`)، یعنی «بازگشت کامل به حالت اولیه». فقط در
  حالت عادی (هیچ‌کس Focus نیست) و روی تصویری که خودش الان Zoom‌شده،
  اولین Tap استثنائاً فقط همان Zoom را می‌بندد بدون Focus‌کردن همه
  (رفتار مرسوم Viewer عکس، تا کاربر نتواند با یک Tap ساده از حالت
  Zoom مستقیم به Focus کل گالری بپرد).
...
- **Pinch-Zoom-and-Pan واقعی (جایگزین طراحی اولیه که Pinch را معادل
  ...
  Focus‌شده ۸۰vw، مستقل از این‌که Focus مشترک فعال است یا نه) با
  `transform: translate() scale()` روی خود `<Image>` بزرگ‌نمایی
  می‌شود (Scale بین ۱ تا ۳، `clampZoomScale`). وقتی Scale > ۱، با
  یک انگشت می‌توان تصویر را در همان کانتینر Pan/جابه‌جا کرد تا نقاط
  مختلف دیده شود — جابه‌جایی همیشه با `clampPanOffsetPx` محدود به
  لبه‌های تصویر است (فرمول `containerSize*(scale-1)/(2*scale)`،
  بدون فاصله خالی). وقتی تصویری Zoom‌شده، اسکرول افقی خود گالری
  موقتاً `overflow-hidden` می‌شود تا با Pan یک‌انگشتی تداخل نکند.
...
- **عمداً اضافه نشد (طبق دستور صریح کارفرما، بعداً در فازهای جدا اضافه شد/می‌شود):**
  - `technicalDescription` (متن آزاد فنی) — در فاز بعدی‌تر اضافه شد
    (نگاه کنید `ProductTechnicalNotes` پایین‌تر در این سند).
  - «محصولات مشابه» و «همراه با این محصول خریده شده است» — در فاز
    بعدی‌تر اضافه شدند (نگاه کنید `RelatedProductsCarousel`
    پایین‌تر). نظرات/ریتینگ هنوز اضافه نشده‌اند — کارفرما گفت آن دو
    فاز جداگانه‌اند.
...
**اصلاح دوم بخش قیمت طبق بازخورد کارفرما (بلافاصله بعد از اصلاح بالا):**

- بک‌گراند/سایه/گوشه‌گرد کارت قیمت کامل حذف شد — دیگر یک کارت
  نیست، فقط یک بلوک بدون بک‌گراند؛ همه محتوایش (قیمت، Badge تخفیف،
  قیمت خط‌خورده، Pill صرفه‌جویی) وسط‌چین شد (`items-center
  justify-center text-center`).
- ردیف وضعیت موجودی («نقطه سبز + موجود در انبار» / «ناموجود») کامل
  از نمایش حذف شد. منطق خودِ `isOutOfStock` دست‌نخورده ماند —
  همچنان Stepper تعداد را در ناموجود بودن پنهان می‌کند و دکمه
  افزودن به سبد را در `ProductAddToCartBar` غیرفعال/«ناموجود»
  می‌کند؛ فقط آن سطر *نمایشی* از کارت قیمت حذف شد.
- `ProductVariantDetails`: ترتیب بخش رنگ عوض شد — قبلاً «(دایره)
  رنگ: نام» بود؛ حالا «رنگ: (دایره) نام» — یعنی دایره رنگی *بعد*
  از دو‌نقطه «:» می‌آید، نه قبل از کلمه «رنگ». دور دایره یک رینگ
  سفید (`border-2 border-white`) + یک رینگ نازک تیره کم‌رنگ
  (`ring-1 ring-black/10`) اضافه شد تا رنگ‌های روشن هم روی
  بک‌گراند سفید/خاکستری روشن صفحه به‌وضوح دیده شوند.
- تست‌ها: TypeScript ✅، ESLint ✅، Vitest (بدون تغییر در تست‌ها —
  فقط تغییر ظاهری/JSX، منطق پایه یکسان ماند، ۴۴۵ تست)، Build ✅.

**جدول ویژگی‌های فنی (طبق دستور دقیق کارفرما — اضافه‌شده به Product Details):**

- **`ProductTechnicalSpecsCard`** (کامپوننت جدید): از
  `Product.technicalSpecifications` (فیلدهای `key`/`value`) —
  فقط اگر محصول حداقل یک ردیف داشته باشد رندر می‌شود.
- پیاده‌سازی بصری با CSS Grid دو‌ستونه (`grid-cols-[1fr_2fr]` — عنوان
  ⅓، مقدار ⅔)، نه یک `<table>` HTML واقعی: چون هم گوشه‌گردشدن
  مستقل هر خانه، هم ۴px فاصله دقیق بین همه خانه‌ها (`gap-1`)، روی
  یک Table واقعی (به‌خاطر مدل Rendering خودِ Table/`border-spacing`)
  قابل‌اطمینان نبود؛ خروجی بصری همان جدولی است که کارفرما خواسته.
- **اصلاح‌شده طبق بازخورد کارفرما (برعکسِ نسخه اول):** اول ناحیه
  Grid خاکستری بود و خانه‌ها سفید؛ حالا برعکس — ناحیه Grid سفید
  می‌ماند (هم‌رنگ خودِ کارت) و خانه‌ها بک‌گراند خاکستری روشن
  (`bg-gray-100`) دارند؛ فاصله ۴px بینشان حالا سفیدِ زمینه کارت را
  نشان می‌دهد.
- خانه‌ها گوشه‌گرد. ستون عنوان وسط‌چین (`text-center`)، ستون مقدار
  راست‌چین (`text-right` — همان جهت تحت‌اللفظی «راست‌چین»
  درخواست‌شده، نه `text-start` منطقی).
- «هدر سراسری» بخش (نه هدر هر ستون) — آیکن `ListChecks` در دایره
  رنگی + عنوان Bold «ویژگی‌های فنی»، هم‌الگو دقیق با هدر
  `ProductDescriptionCard`؛ کل کارت هم بک‌گراند سفید + گوشه‌گرد است.
- `get-product-detail.ts` گسترش یافت: `technicalSpecifications`
  اضافه شد به `PRODUCT_DETAIL_FIELDS` + خروجی. هنوز عمداً اضافه
  نشده: `Product.technicalDescription` (متن آزاد فنی، جدا از همین
  جدول) — کارفرما فقط دستور جدول را داد، منتظر دستور بعدی برای آن.
- تست‌ها: TypeScript ✅، ESLint ✅، Vitest (۴۴۵ تست، بدون تغییر — این
  کامپوننت صرفاً نمایشی است، بدون منطق خالص جدید برای تست)، Build ✅.

**تکمیل Product Details طبق دستور دقیق بعدی کارفرما (دسته‌بندی/برند لینک‌شده، محصولات مرتبط، رفع باگ Variant، ملاحظات فنی):**

- **`ProductInfoHeader` گسترش یافت:**
  - Chip دسته‌بندی حالا `<Link href="/categories/{slug}">` است، نه
    فقط متن. اگر دسته‌بندی محصول یک والد هم داشته باشد (این پروژه
    حداکثر دو سطح دسته‌بندی دارد — `Category.parentId`)، هر دو Chip
    کنار هم نمایش داده می‌شوند؛ `getProductDetailBySlug` حالا
    `categories: {name,slug}[]` برمی‌گرداند (عنصر ۰ = والد اگر
    باشد، آخرین = خودِ دسته‌بندی)، نه یک `categoryName` تکی.
  - **نکته مهم:** صفحه مقصد `/categories/{slug}` هنوز در Storefront
    ساخته نشده — این یک حفرهٔ *از قبل موجود* در پروژه بود
    (`category-shortcuts.tsx` صفحه اصلی هم از قبل به همین مسیر
    لینک می‌داد بدون این‌که آن صفحه وجود داشته باشد)، نه چیزی که
    همین‌جا ایجاد شده باشد. ساختن خودِ آن صفحه (فهرست محصولات یک
    دسته‌بندی) یک ماژول کاملاً جدا و هم‌اندازه صفحه محصول است؛
    منتظر دستور/تأیید جدا.
  - Chip برند جدید («برند: نام‌برند»، فقط اگر `Product.brand`
    مقدار داشته باشد) — `get-product-detail.ts` حالا `brand` را هم
    Populate می‌کند (`brandName` در خروجی).
- **باگ رفع‌شده — `ProductVariantSelector` با یک Variant کامل مخفی
  می‌شد:** طبق بازخورد کارفرما، آن Pill تنها جای نمایش واحد فروش
  (`unit`) است؛ حذف کامل بخش یعنی آن اطلاعات هیچ‌جا دیده نمی‌شد.
  حالا با هر تعداد Variant (حتی ۱) رندر می‌شود.
- **عنوان جدول ویژگی‌ها عوض شد:** «ویژگی‌های فنی» ← «ویژگی‌های
  محصول» (فقط متن روی صفحه، خودِ Component/فایل همان
  `ProductTechnicalSpecsCard` ماند).
- **دو ردیف محصول مرتبط جدید، بعد از جدول ویژگی‌های محصول:**
  - **`RelatedProductsCarousel`** (کامپوننت عمومی جدید، هم‌ساختار
    دقیق با ردیف‌های صفحه اصلی — همان `SectionHeader` + همان اسکرول
    افقی `ProductCard` — اما بدون `seeAllHref`، پس دکمه «مشاهده
    بیشتر» ندارد؛ یک کامپوننت مشترک برای هر دو ردیف زیر، نه دو
    کپی تقریباً یکسان).
  - **«محصولات مشابه»** (`getSimilarProductCards` در
    `get-related-products.ts` جدید): رتبه‌بندی بر اساس شباهت
    *عنوان* — یک Text Index مونگو تازه روی `Product.title`
    (`ProductSchema.index({title:"text"})`) + `$meta:"textScore"`.
    حداکثر ۱۰ محصول.
  - **«همراه با این محصول خریده شده است»**
    (`getFrequentlyBoughtTogetherCards`): بر اساس Orderهای واقعی —
    هم‌الگو دقیق با `getBestSellerProductCards` در
    `homepage-products.ts` (Aggregate روی `Order`، سفارش‌های
    لغوشده/مرجوعی حذف)؛ این‌جا محصولاتی که در همان سفارش‌های این
    محصول هم بوده‌اند، بر اساس تعداد هم‌سفارشی مرتب می‌شوند.
    حداکثر ۱۰ محصول.
  - چند تابع/نوع private در `homepage-products.ts` برای استفاده
    مجدد Export شدند (نه Duplicate): `PRODUCT_CARD_FIELDS`،
    `LeanProductForCard`، `buildColorsMap`، `isDisplayable`،
    `toDisplayableCards`.
- **`ProductTechnicalNotes`** (کامپوننت جدید): فقط اگر
  `Product.technicalDescription` (متن آزاد، جدا از `description` و
  از جدول ویژگی‌ها — دستور صریح قبلی کارفرما همین یکی را برای بعداً
  نگه داشته بود) مقدار داشته باشد — یک خط ساده *بدون بک‌گراند*، در
  انتهای صفحه (بعد از دو ردیف محصول مرتبط بالا): «ملاحظات فنی: …».
- تست‌ها: TypeScript ✅، ESLint ✅، Vitest (۴۴۵ تست — بدون تغییر؛
  Queryهای جدید DB-touching هستند، طبق قانون پروژه تست خالص
  ندارند)، Build ✅.

**باگ Production رفع‌شده — خطای سرور واقعی روی صفحه محصول بعد از همین Deploy بالا:**

کارفرما اسکرین‌شات خطای واقعی «This page couldn't load / A server
error occurred» را درست روی صفحه جزئیات محصول فرستاد.

- **علت:** `ProductSchema.index({title:"text"})` تازه اضافه‌شده بود.
  Mongoose ایندکس‌های جدید را در پس‌زمینه می‌سازد (`autoIndex`
  پیش‌فرض `true`)، نه فوری/همزمان با شروع فرآیند. اولین
  درخواست‌هایی که درست همان لحظه اول بعد از Deploy (قبل از
  تمام‌شدن ساخت Index) به `getSimilarProductCards` می‌رسیدند، با
  خطای مونگو «text index required for `$text` query» رد می‌شدند —
  یک Race Condition کلاسیک روی هر Deploy تازه (و روی هر Cold Start
  تازه سرورلس، چون Cache اتصال Mongoose هم‌فرآیندی است، نه
  سراسری).
- **رفع مستقیم:** `await Product.init()` در ابتدای
  `getSimilarProductCards` — طبق مستندات خودِ Mongoose، این Promise
  دقیقاً وقتی Resolve می‌شود که ساخت Index (اگر در حال انجام باشد)
  تمام شده باشد؛ صدازدن چندبارهٔ آن (هر درخواست) بی‌خطر و
  تقریباً بی‌هزینه است.
- **رفع سیستمی/مهم‌تر (Resilience):** در `page.tsx`، هر دو Query
  ردیف محصول مرتبط حالا با `Promise.allSettled` اجرا می‌شوند، نه
  `Promise.all` — یک خطای گذرا در هرکدام (این یکی یا هر خطای
  آینده‌ای مثل این) دیگر نمی‌تواند کل صفحه محصول (گالری/قیمت/افزودن
  به سبد) را از کار بیندازد؛ فقط همان ردیف خالی می‌ماند و خطا
  `console.error` می‌شود، نه Throw. این دو ردیف تکمیلی‌اند، نه بخش
  حیاتی صفحه — اصلی که از این پس برای هر Query غیرحیاتی مشابه در
  این صفحه باید رعایت شود.
- تست‌ها: TypeScript ✅، ESLint ✅، Vitest (۴۴۵ تست، بدون تغییر)،
  Build ✅.
...
- ✅ **صفحه «سبد خرید» Storefront ساخته شد (`/cart`).** طبق دو
  تصویر رفرنس کارفرما (اپ میوه‌فروشی) اما با طراحی مختص این پروژه —
  کارفرما صراحتاً خواسته بود ظاهر «به‌مراتب زیباتر و حرفه‌ای‌تر» از
  رفرنس باشد، نه کپی آن.

  **تصمیم معماری مهم:** چون `POST /api/v1/checkout` از قبل کامل
  ساخته شده بود (آدرس + روش پرداخت + Cart در یک درخواست → سفارش)،
  این صفحه به‌جای دو صفحه مجزای «سبد خرید»/«تسویه‌حساب» (که بند ۶۹
  در بخش ۵ فهرست کرده بود)، دقیقاً هم‌الگوی رفرنس کارفرما **یک صفحه
  واحد** طراحی شد: آدرس تحویل + اقلام + کد تخفیف + روش پرداخت + ثبت
  نهایی، همه در `/cart`. اگر بعداً کارفرما دو صفحه مجزا بخواهد، این
  یک Rewrite کامل UI می‌خواهد (نه فقط افزودن یک صفحه) — تصمیم آگاهانه
  ثبت شده باشد.

  **روش پرداخت (تفاوت اصلی با رفرنس):** رفرنس کارفرما «پرداخت در
  محل» داشت؛ این پروژه به‌جایش دو گزینه دارد که هر دو در واقع همان
  `paymentMethod: "online"` سمت Backend‌اند، فقط با فلگ `useWallet`
  فرق می‌کنند (نگاه کنید `initiateOrderPayment` در بخش ۹/۱۳):
  - «پرداخت آنلاین» — کل مبلغ از درگاه زرین‌پال.
  - «پرداخت ترکیبی (کیف پول + آنلاین)» — تا سقف موجودی واقعی کیف
    پول کاربر کسر می‌شود، باقیمانده به درگاه می‌رود؛ اگر کل مبلغ را
    کیف پول پوشش دهد، اصلاً به درگاه نیازی نیست (`paidFromWallet`).
  - گزینه `cash` (پرداخت در محل) و روش `split`/بیعانه (که به یک
    درصد تأییدشده توسط ادمین نیاز دارد) **عمداً در این صفحه
    Self-Service ساخته نشدند** — تصمیم معماری‌ای که این Task حل
    نکرد؛ اگر لازم شد، یک تصمیم/Task جداست.

  **هزینه ارسال:** چون سند Audit صراحتاً می‌گوید موتور محاسبه خودکار
  هنوز وجود ندارد، صفحه یک عدد ساختگی نشان نمی‌دهد — همان `۰`/رایگان
  واقعی فعلی سیستم را نشان می‌دهد (هم‌الگوی اصل «صفر واقعی به‌جای
  Placeholder» که قبلاً برای بج سبد خرید/اعلان هم رعایت شده بود).

  **آدرس تحویل:** آدرس پیش‌فرض کاربر (از همان `Address` که در
  `/account/addresses` مدیریت می‌شود) مستقیم در Server Component
  صفحه خوانده و نمایش داده می‌شود؛ لینک «تغییر» به همان صفحه
  می‌رود (یک Selector اختصاصی داخل خود Cart ساخته نشد — Scope این
  Task نبود). اگر کاربر هیچ آدرسی نداشته باشد، دکمه «افزودن آدرس
  تحویل» به‌جای کارت آدرس نشان داده می‌شود و دکمه ثبت سفارش غیرفعال
  می‌ماند.

  **رفتار بعد از ثبت سفارش:**
  - اگر `payment.paymentUrl` برگردد → Redirect کامل مرورگر به
    زرین‌پال (دقیقاً مثل `/account/wallet/topup`).
  - اگر `payment.paidFromWallet` باشد (کل مبلغ از کیف پول پوشش داده
    شد) → Redirect به همان صفحه عمومی موجود `/payment/result` با
    `status=success` — صفحه مستقل موفقیت سفارش تازه ساخته نشد، چون
    این صفحه از قبل دقیقاً همین نقش را برای Callback زرین‌پال هم
    ایفا می‌کند (بدون Duplicate).
  - اگر سفارش ثبت شد ولی شروع پرداخت خطا داد (`paymentError`) →
    پیام Inline در همان صفحه Cart (که حالا خالی نشان داده می‌شود،
    چون Backend سبد را در هر دو حالت پاک می‌کند) — صفحه «سفارش‌های
    من» هنوز ساخته نشده (بند ۶۹) تا کاربر بعداً از آن‌جا پیگیری کند.

  **فایل‌های جدید:**
  - `src/app/(storefront)/cart/page.tsx` — Server Component: Auth
    Guard + خواندن مستقیم Cart/آدرس پیش‌فرض/موجودی کیف پول از DB
    (نه Fetch به API خودش، هم‌الگوی Hero Slider/صفحه کیف پول).
  - `src/components/storefront/cart-page-client.tsx` — تعامل کامل:
    تغییر تعداد (`PATCH /api/v1/cart/items/:itemId`)، حذف
    (`DELETE`)، اعمال/حذف کد تخفیف (`POST`/`DELETE
    /api/v1/cart/coupon`)، انتخاب روش پرداخت، ثبت نهایی
    (`POST /api/v1/checkout`).
  - `src/components/storefront/cart-item-row.tsx` — ردیف هر Item
    (تصویر/عنوان/واحد/Stepper تعداد/قیمت با تخفیف واقعی/وضعیت «در
    دسترس نیست» مستقیم از `isAvailable`/`unavailableReason`).
  - `src/lib/cart/wallet-split-preview.ts` (+ تست) — تابع خالص
    پیش‌نمایش تقسیم مبلغ بین کیف پول/درگاه در UI؛ محاسبه واقعی/نهایی
    همیشه دوباره در `initiateOrderPayment` انجام می‌شود، این تابع
    فقط برای نمایش عدد قبل از ثبت سفارش است.
  - `storefront-chrome.tsx`: مسیر `/cart` به
    `HIDDEN_BOTTOM_BAR_PREFIXES` اضافه شد — صفحه Cart نوار پایین
    چسبان اختصاصی خودش («ثبت نهایی سفارش») را دارد، هم‌الگوی صفحه
    محصول.

  **باقی‌مانده برای بعد (عمداً در این Task انجام نشد):** بج تعداد
  سبد خرید در `MobileBottomBar` سراسری هنوز همیشه `۰` نمایش داده
  می‌شود (نگاه کنید بخش ۲ Phase 1) — حالا که Cart واقعی/صفحه‌اش
  ساخته شده، وصل‌کردن این بج به تعداد واقعی یک قدم طبیعی بعدی است،
  اما چون به یک تصمیم زیرساختی (خواندن Cart Count در کجای Layout)
  نیاز دارد، این Task آن را تغییر نداد.

  تست جدید: `wallet-split-preview.test.ts` (۵ تست). مجموع تست‌ها
  اکنون ۴۵۰. TypeScript/ESLint/Vitest/Build همه سبز.

- ✅ **باگ Production دوباره گزارش‌شد — همان علامت («This page
  couldn't load / A server error occurred»)، دوباره درست روی صفحه
  جزئیات محصول، بعد از Deploy بالا.** این علامت قبلاً یک‌بار (بند
  Race شرط ایندکس Text، بالاتر) رفع اعلام شده بود؛ چون دوباره اتفاق
  افتاد، آن تشخیص یا کامل نبود یا علت واقعی این‌بار چیز دیگری بود —
  بدون دسترسی به Log واقعی Vercel/دیجست خطا، علت دقیق قطعی نیست، اما
  با بازبینی کامل `getProductDetailBySlug` دو ضعف واقعی (نه فرضی) پیدا
  و رفع شد که هرکدام به‌تنهایی می‌توانند دقیقاً همین رفتار (کرش کل
  صفحه فقط برای یک محصول خاص، نه همه محصولات) را توضیح دهند:

  - `product.images.map(...)`, `product.variants.map(...)`,
    `variant.attributes.map(...)`, `product.technicalSpecifications.map(...)`
    و ورودی `pickRepresentativeVariant(product.variants)` همه بدون
    محافظت در برابر `undefined` بودند — اگر سند یک محصول خاص در DB
    (مثلاً از قبل از افزودن یک فیلد، یا ویرایش دستی) یکی از این
    آرایه‌ها را نداشته باشد، `.map`/`.length` روی `undefined` بلافاصله
    Exception پرتاب می‌کند و کل Render صفحه (نه فقط یک بخش) از کار
    می‌افتد. رفع شد: همه‌جا با `?? []` محافظت شدند.
  - **شکاف مهم‌تر و سیستمی‌تر:** کل گروه Route با پرانتز
    `(storefront)` اصلاً هیچ `error.tsx`ای نداشت (برخلاف
    `(dashboard)` که چند صفحه‌اش دارد) — یعنی *هر* خطای رفع‌نشده در
    *هر* صفحه Storefront (نه فقط صفحه محصول) مستقیم به صفحه خام
    عمومی مرورگر می‌رسید، نه یک صفحه هم‌ساختار با طراحی سایت با دکمه
    «تلاش دوباره». `src/app/(storefront)/error.tsx` اضافه شد —
    Client Component با `console.error` (برای دیدن دیجست بعدی در Log
    واقعی) + دکمه Retry، هم‌الگو با `(dashboard)/dashboard/error.tsx`
    اما با ظاهر/توکن‌های Storefront. این خودِ علت ریشه‌ای احتمالی
...
- ✅ **صفحه کامل `/orders`** — لینک «سفارش‌های من» در `/account`
  ...
  **صفحه جزئیات سفارش `/orders/[id]` — بلافاصله بعد از تأیید همین
  Session اضافه شد** (کارفرما با لینک نمونه یک سفارش واقعی خواست):
  - `src/lib/storefront/get-order-detail.ts`:
    `getOrderDetailForCustomer(orderId, userId)` — با
    `mongoose.isValidObjectId` معتبرسنجی می‌شود، فقط اگر
    `order.customer` واقعاً همان کاربر باشد چیزی برمی‌گرداند (در غیر
    این صورت `null` → صفحه `notFound()` صدا می‌زند؛ ۴۰۴ نه ۴۰۳، تا
    وجود سفارشِ کاربر دیگر هم لو نرود). فیلد `changedBy` هر ردیف
    `statusHistory` و فیلد کلی `order.notes` عمداً Select/برگردانده
    نمی‌شوند — نام کارمند و یادداشت داخلی ادمین اطلاعاتی نیستند که
    باید به مشتری نشان داده شوند؛ فقط یادداشتِ خودِ هر تغییر وضعیت
    (که معمولاً برای توضیح یک استثنا مثل لغو نوشته می‌شود) نمایش
    داده می‌شود. + `formatOrderDiscountLabel` (تابع خالص، همان
    الفاظ Dashboard: «تخفیف کد X» / «پاداش پرداخت آنلاین/ترکیبی»،
    تست‌شده).
  - **رفوند کیف پول/کد پیگیری بانکی رفرنس کارفرما عمداً پیاده
    نشد** — این پروژه اصلاً چنین فیلدهایی ندارد (`Order` مدل هیچ
    فیلد Refund ندارد؛ لغو سفارش طبق
    `api/v1/orders/[id]/status/route.ts` فقط موجودی را برمی‌گرداند،
    نه پولی را). به‌جایش بنر وضعیت از یادداشت واقعی همان تغییر
    وضعیت (اگر ادمین نوشته باشد) یا یک توضیح پیش‌فرض مناسب همان
    وضعیت استفاده می‌کند — بدون اطلاعات ساختگی.
  - `src/components/storefront/order-status-banner.tsx`: بنر رنگی
    بالای صفحه (سبز/قرمز/آبی/کهربایی بر اساس وضعیت) + همان توضیح.
  - صفحه شامل ۴ کارت: بنر وضعیت، اطلاعات سفارش (تاریخ/آدرس تحویل/
    روش پرداخت)، کالاها (هر قلم با رنگ/ویژگی/تعداد/قیمت + ریز
    قیمت: جمع اقلام/تخفیف/ارسال/مبلغ کل، و برای سفارش «ترکیبی»
    سهم درگاه/کیف پول جدا)، و تاریخچه وضعیت (Timeline).
  - تاریخ/زمان همه‌جا فقط **تاریخ** نشان داده می‌شود (بدون ساعت) —
    عمداً، چون `formatJalali` موجود پروژه Time-Safe است (روی UTC
    Anchor می‌شود) ولی نسخه ساعت‌دار (`isoToJalaliDateTime`) از
    ساعت *محلی همان Runtime* می‌خواند؛ چون این صفحه یک Server
    Component است (نه مرورگر کاربر)، نمایش ساعت آن می‌توانست با
    منطقه زمانی سرور (احتمالاً UTC) به‌جای ساعت ایران اشتباه باشد —
    پس برای درستی، فقط تاریخ نشان داده شد (هم‌الگو با
    `WalletTransactionRow` که از قبل همین تصمیم را داشت).
  - «سفارش مجدد» رفرنس عمداً اضافه نشد — یک قابلیت جدا (بررسی
    موجودی/فعال بودن هر Variant و افزودن به سبد) است، نه بخشی از
    خودِ «نمایش جزئیات»؛ می‌تواند ماژول بعدی باشد.
  - تست جدید: `get-order-detail.test.ts` (۳ تست
    `formatOrderDiscountLabel`). TypeScript ✅، ESLint ✅، Vitest
    (۴۷۱ تست، ۳ تست جدید) ✅، Build ✅ (`/orders/[id]` در خروجی
    Build دیده می‌شود).

  **بازطراحی کارت «کالاها» (به درخواست صریح کارفرما بلافاصله بعد از
  تأیید اول):**
  - ویژگی‌های فنی Variant (`attributes`: «شانه: ۱۲۰۰» و مانند آن)
    کاملاً از این صفحه حذف شد — دیگر نه Select می‌شود نه در نوع
    `OrderDetailItem` وجود دارد. این اطلاعات همچنان کامل داخل
    Dashboard (`order-detail-card.tsx`) هست؛ فقط از دید مشتری
    برداشته شد.
  - هر قلم حالا تصویر کوچک محصول (`size-14`) + لینک واقعی به
    `/products/[slug]` دارد (هم روی عکس هم روی عنوان). این دو داده
    (تصویر/Slug) روی خودِ سفارش ذخیره نمی‌شوند (Snapshot سفارش هیچ
    عکسی ندارد)، پس `getOrderDetailForCustomer` حالا با یک Query
    اضافه (`Product.find` روی `productId`های سفارش، فقط
    `status: "published"`) آن‌ها را زنده می‌خواند.
  - **محصول حذف‌شده/غیرفعال‌شده:** اگر محصولی دیگر `published` نباشد
    (حذف کامل یا هر تغییر وضعیت دیگر)، Query بالا آن قلم را برنمی‌گرداند
    → `item.product` می‌شود `null`. صفحه در این حالت لینک نمی‌دهد
    (وگرنه مستقیم به ۴۰۴ صفحه محصول می‌رفت)، به‌جایش یک جعبه خاکستری
    با آیکون «بدون تصویر» + برچسب «دیگر در فروشگاه موجود نیست» نشان
    می‌دهد؛ عنوان/رنگ/تعداد/قیمت (که همه از Snapshot خودِ سفارش‌اند،
    نه از محصول زنده) دقیقاً مثل قبل درست باقی می‌مانند.
  - رنگ حالا به‌صورت یک دیسک رنگی + اسم نشان داده می‌شود، نه فقط
    متن. `Order` مدل فقط اسم رنگ (`colorName`) را Snapshot می‌گیرد،
    نه کد Hex؛ چون مجموعه `Color` یک نام سراسری یکتا دارد
    (`models/Color.ts`)، کد از همان‌جا با تطبیق نام پیدا می‌شود —
    این روش حتی اگر محصول/Variant اصلی حذف شده باشد هم کار می‌کند
    (چون به محصول وابسته نیست، فقط به نام رنگ).
  - `src/components/storefront/order-detail-item-row.tsx`: کامپوننت
    جدید برای همین ردیف (قبلاً JSX این ردیف مستقیم داخل `page.tsx`
    بود، با رشد منطق Fallback/لینک به یک Component مستقل منتقل شد).
  - فاصله‌ها/اندازه فونت کل صفحه یک درجه شیک‌تر شد: Padding کارت‌ها
    از `p-4` به `p-5`، عنوان هر کارت از `text-xs` به `text-sm`،
    فاصله بین کارت‌ها از `space-y-4` به `space-y-5`.
  - فایل‌های تغییریافته: `get-order-detail.ts`،
    `order-detail-item-row.tsx` (جدید)، `orders/[id]/page.tsx`.
    بدون تغییر در مدل‌ها/API. TypeScript ✅، ESLint ✅، Vitest
    (۴۷۱ تست، بدون تغییر) ✅، Build ✅.

  **اصلاح چیدمان «اطلاعات سفارش» و «کالاها» (فیدبک روی خودِ محیط
  Production، سفارش #۱۰۰۰۲):** کارفرما گفت اطلاعات سفارش یک‌رنگ/
  تودرتو و نامرتب است، آدرس جالب نیست، و عنوان کالا بین عکس و قیمت
  Truncate می‌شد («فرش ۷۰۰ شانه طر...»).
  - **اطلاعات سفارش:** به‌جای سه ردیف هم‌رنگ/هم‌وزن (آیکون+جمله)،
    حالا تاریخ/روش پرداخت دو «آمار» کنار هم‌اند (آیکون در دایره
    خاکستری + برچسب کم‌رنگ بالای مقدار پررنگ — Component محلی
    `InfoStat`، فقط همین صفحه). آدرس از این دو جدا شد: یک زیر-بلوک
    مجزا با پس‌زمینه `bg-gray-50`، سرتیتر «آدرس تحویل» با آیکون
    رنگی، بعد نام+تلفن (پررنگ) در یک خط، آدرس کامل در خط بعد، کد
    پستی در خطی جداگانه کم‌رنگ‌تر — دیگر یک بلوک متن به‌هم‌چسبیده
    نیست.
  - **کالاها:** ریشه مشکل این بود که عنوان در همان ردیف افقیِ عکس و
    قیمت کل جا می‌گرفت، پس هرچه عنوان بلندتر، بیشتر Truncate
    می‌شد. الان قیمت کل از آن ردیف بیرون آمد: عنوان کاملاً Wrap
    می‌شود (بدون Truncate) در ستون کنار عکس، رنگ (اگر باشد) دقیقاً
    زیر عنوان، و یک ردیف پایینی جدا دارد «تعداد × قیمت واحد» سمت
    راست + «قیمت کل» سمت چپ — دقیقاً ترتیبی که کارفرما خواسته بود.
  - فایل‌های تغییریافته: `orders/[id]/page.tsx` (اطلاعات سفارش +
    `InfoStat` محلی)، `order-detail-item-row.tsx` (بازنویسی چیدمان).
    بدون تغییر در `get-order-detail.ts`/مدل‌ها/API — فقط چیدمان.
...
- [ ] بعد از صفحه اصلی: Products/Category/Product Detail/Search/
  Login-OTP/Account/Orders/Address (بند ۶۹)
- [x] Cart (+ عملاً Checkout هم — نگاه کنید بخش ۳): صفحه واحد
  `/cart` روی `POST /api/v1/checkout` موجود ساخته شد، بدون
  بازنویسی مدل‌های `Order`/`Payment`
...
- **(منسوخ — رفع شد) Address Book مستقل مشتری:** این ورودی می‌گفت
  دفترچه آدرس مستقل هنوز طراحی نشده؛ از آن زمان `/account/addresses`
  کامل ساخته شد و صفحه `/cart` هم مستقیم از همان آدرس پیش‌فرض
  استفاده می‌کند (نگاه کنید بخش ۳، Cart).
...
- **بدون موتور محاسبه خودکار هزینه ارسال:** `POST /api/v1/checkout`
  ...
  از روی فاصله هزینه واقعی را حساب نمی‌کند. صفحه `/cart` صادقانه
  «رایگان» (یعنی همان `۰` واقعی فعلی) نشان می‌دهد و همیشه
  `shippingCost: 0` می‌فرستد — وقتی موتور محاسبه ساخته شد، همین یک
  خط UI باید به مقدار واقعی وصل شود.
...
- **مهم — محدودیت هر کاربر (`perUserLimit`) Coupon داخل یک
  Transaction نیست:** فقط یک‌بار قبل از رزرو ظرفیت بررسی می‌شود.
  **به‌روزرسانی:** از وقتی صفحه `/cart` ساخته شد، Coupon دیگر فقط از
  Dashboard در دسترس نیست — مشتریان واقعی هم‌زمان می‌توانند از
  چندین مرورگر Checkout بزنند، پس این ریسک دیگر تئوریک نیست. قبل از
  Traffic واقعی، باید با یک الگوی اتمیک‌تر (مثلاً Unique Index
  ترکیبی روی `{coupon, user}` وقتی `perUserLimit === 1`) بازبینی
  شود.
...
- [ ] محدودیت هر کاربر (`perUserLimit`) Coupon را به یک الگوی
  اتمیک‌تر ارتقا بده — دیگر فرضی نیست، چون Coupon از طریق صفحه
...
- [x] صفحه «سفارش‌های من» (`/orders`) برای Storefront — ساخته شد
  (بخش ۳، بعد از بخش In Progress قبلی)
- [x] صفحه جزئیات تک‌سفارش (`/orders/[id]`) — ساخته شد (بخش ۳،
  همان بخش «سفارش‌های من»)
