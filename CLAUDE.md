...
**Phase 4: Special Offers Carousel («شگفت‌انگیزها») — یک دور بازخورد
دیداری اعمال شد (هنوز Mock).**

- `SectionHeader`: حالا `titleColorClassName` و `seeAllLabel` هم
  قابل بازنویسی‌اند (برای هماهنگی رنگ برند هر بخش و برچسب متفاوت
  «مشاهده بیشتر»/«مشاهده همه»).
- `AmazingOffersSection`: آیکون `Sparkles` + عنوان + «مشاهده بیشتر»
  همگی با رنگ برند اختصاصی بخش (توکن جدید `--sf-cherry`/
  `--sf-cherry-soft` در `globals.css` — قرمز آلبالویی، برای فوریت/
  تخفیف آنی). `seeAllHref` موقتاً به `/products/amazing-offers`
  اشاره می‌کند (صفحه هنوز ساخته نشده).
- `AmazingOfferCard`: برچسب بالای کارت وسط‌چین + رنگ `--sf-cherry`،
  Progress Bar هم `--sf-cherry`. ترتیب ردیف قیمت/درصد برعکس شد: بج
  درصد → راست، بلوک قیمت (چپ‌چین) → چپ. قیمت خط‌خورده دیگر کلمه/گلیف
  تومان ندارد (`formatNumber`، فقط عدد). فونت قیمت/گلیف کوچک‌تر شد و
  گلیف تومان در `<span>` جدا، کمی بالاتر از خط پایه (`relative -top-0.5`)
  و هم‌ردیف با عدد (`whitespace-nowrap`، دیگر روی دو خط نمی‌شکند).
  شمارش معکوس: بین اعداد و «:» فاصله اضافه شد، وزن فونت یک درجه
  بیشتر شد (`font-semibold`).
- **تصاویر Mock:** دو مورد از چهار تصویر Placeholder (که حاشیهٔ خالی
  زیادی داخل خود عکس داشتند و باعث می‌شد محصول در قاب ۳:۴ کوچک‌تر از
  بقیه به‌نظر برسد) با دو نمونهٔ Full-Bleed تکراری جایگزین شدند. خودِ
  قاب تصویر همیشه ابعاد ثابت داشت (Aspect Ratio روی Container)؛
  مشکل فقط از انتخاب تصویر Mock بود، نه چیدمان.
- **گلیف «تومان»:** بعد از بررسی مجدد GSUB فونت، سه واریانت پیدا شد
  («ء»×۱/۲/۳ → گلیف‌های `tomaan`/`tomaan.001`/`tomaan.002`). حالا از
  `tomaan.001` («ء»×۲) استفاده می‌شود چون تقریباً کامل روی خط پایه
  می‌نشیند (بر خلاف `tomaan` که تا ۰.۲۵em زیر خط پایه می‌آید) و برای
  هم‌ردیف‌شدن با عدد قیمت مناسب‌تر است. `TOMAN_GLYPH` (فقط خودِ گلیف،
  بدون عدد) از `format.ts` Export شد تا در `<span>` مستقل با سایز/
  موقعیت خودش قابل استفاده باشد؛ `formatTomanGlyph` (عدد+گلیف با هم)
  هم برای استفاده‌های ساده‌تر باقی ماند.

تست‌ها: TS/ESLint/Vitest (۲۹۸ تست، ۱ تست جدید برای `TOMAN_GLYPH`)/
Build همه سبز.
