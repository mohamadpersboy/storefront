...
export default async function PaymentResultPage({
  ...
  searchParams: Promise<{ status?: string; orderNumber?: string; amount?: string }>;
}) {
  const { status, orderNumber, amount } = await searchParams;
  ...
  const config = {
    success: {
      ...
      description: "رسید پرداخت برای شما ثبت شد. وضعیت سفارش از طریق پیامک اطلاع‌رسانی می‌شود.",
    },
    failed: {
      icon: XCircle,
      color: "text-danger",
  ...
  return (
    <main className="flex min-h-screen items-center justify-center bg-surface-subtle px-4">
      <div className="w-full max-w-sm rounded-[var(--radius-lg)] border border-border bg-white p-6 text-center shadow-sm">
        ...
        {orderNumber ? (
          <div className="mt-4 flex flex-col gap-1 rounded-[var(--radius-md)] border border-dashed border-border-strong p-3 text-sm">
            <span className="text-muted">
              سفارش #{toPersianDigits(orderNumber)}
            </span>
            {amount ? (
