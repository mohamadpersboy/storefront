...
/**
 * آپلودر تصویر اختیاری برند — مربعی (۱:۱)، دقیقاً هم‌الگو با
 * `CategoryImageUploader` (همان `ImageCropModal` با نسبت ۱:۱ و همان
 * `buildBlurDataUrl`)، فقط با Target متفاوت («brand-image») برای
 * Route امضای آپلود Cloudinary.
 */
export function BrandImageUploader({
  ...
  onChange: (value: BrandImageValue) => void;
}) {
  ...
  async function handleCropped(blob: Blob) {
    ...
    try {
      const signRes = await fetch("/api/v1/uploads/sign", {
        ...
        body: JSON.stringify({ target: "brand-image" }),
      });
      ...
      {value.imageUrl ? (
        <div className="flex items-center gap-3">
          <div className="relative aspect-square w-24 shrink-0 overflow-hidden rounded-[var(--radius-md)] border border-border bg-surface-subtle">
            <Image src={value.imageUrl} alt="تصویر برند" fill className="object-cover" />
          </div>
          <button
            type="button"
