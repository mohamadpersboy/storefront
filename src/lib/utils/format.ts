...
/**
 * فونت IRANYekanX Pro چند گلیف اختصاصی برای نماد «تومان» دارد که با
 * قانون Ligature داخل خود فونت (Feature `rlig` — همیشه در مرورگر
 * فعال است و برخلاف `liga` قابل خاموش‌کردن نیست) فعال می‌شوند: وقتی
 * متن دقیقاً کلمه «تومان» و بلافاصله بعدش یک یا چند نویسه «ء»
 * (Arabic Letter Hamza، U+0621) بیاید، فونت این دنباله را با یک گلیف
 * واحد جایگزین می‌کند. با بررسی مستقیم جدول GSUB فایل فونت سه
 * واریانت پیدا شد: «ء»×۱ (گلیف `tomaan`، تا ۰.۲۵em زیر خط پایه
 * می‌آید)، «ء»×۲ (گلیف `tomaan.001`، تقریباً کامل روی خط پایه
 * می‌نشیند — مناسب هم‌ردیف کردن با عدد قیمت) و «ء»×۳ (`tomaan.002`،
 * باریک‌تر). این نویسه‌ها نباید در جای دیگری از پروژه استفاده شوند،
 * فقط همین‌جا.
 */
const TOMAN_GLYPH_SUFFIX = "\u0621\u0621";

/** فقط خودِ گلیف تومان (بدون عدد) — برای قرار دادن در یک <span> جدا با سایز/راستای مستقل کنار قیمت. */
export const TOMAN_GLYPH = `تومان${TOMAN_GLYPH_SUFFIX}`;
...
export function formatTomanGlyph(amount: number): string {
  return `${tomanFormatter.format(amount)} ${TOMAN_GLYPH}`;
}
